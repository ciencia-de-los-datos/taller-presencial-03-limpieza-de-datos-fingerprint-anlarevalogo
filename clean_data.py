...
def create_fingerprint(df):
    ...
    df['key'] = df['key'].str.split() #Lista de palabras

    #Stemmer son funciones que permiten llevar la palabra a la raiz ej query a queri
...
def generate_cleaned_column(df):
    ...
    keys = df.drop_duplicates(subset = "key", keep = "first") #Elimina llaves duplicadas
    key_dict = dict(zip(keys["key"], keys["text"])) #Trae el texto original limpio de donde se saco la llave
